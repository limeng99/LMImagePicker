...
CGFloat LMScreenWidth;
CGFloat LMScreenScale;

static LMPhotoManager *_manager;
static dispatch_once_t onceToken;
@implementation LMPhotoManager
...
- (instancetype)init {
    if (self = [super init]) {
        LMScreenWidth = [UIScreen mainScreen].bounds.size.width;
        LMScreenScale = 2.0;
        if (LMScreenWidth > 700) {
            LMScreenScale = 1.5;
        }
        CGFloat margin = 4;
        CGFloat itemWH = (LMScreenWidth - 2 * margin - 4) / [LMImagePicker sharedImagePicker].columnNumber - margin;
        AssetGridThumbnailSize = CGSizeMake(itemWH * LMScreenScale, itemWH * LMScreenScale);
        _photoPreviewMaxWidth = [LMImagePicker sharedImagePicker].photoPreviewMaxWidth;
    }
    return self;
}
...
- (void)getCameraRollAlbum:(BOOL)allowPickingVideo allowPickingImage:(BOOL)allowPickingImage needFetchAssets:(BOOL)needFetchAssets completion:(void (^)(LMAlbumModel *model))completion {
    ...
    if (!allowPickingImage) option.predicate = [NSPredicate predicateWithFormat:@"mediaType == %ld",
     ...
     option.sortDescriptors = @[[NSSortDescriptor sortDescriptorWithKey:@"modificationDate" ascending:imagePicker.sortAscendingByModificationDate]];
    if (!imagePicker.sortAscendingByModificationDate) {
        option.sortDescriptors = @[[NSSortDescriptor sortDescriptorWithKey:@"creationDate" ascending:imagePicker.sortAscendingByModificationDate]];
    }
...
- (void)getAllAlbums:(BOOL)allowPickingVideo allowPickingImage:(BOOL)allowPickingImage needFetchAssets:(BOOL)needFetchAssets completion:(void (^)(NSArray<LMAlbumModel *> *))completion{
    LMImagePicker *imagePicker = [LMImagePicker sharedImagePicker];
...
- (void)getAssetsFromFetchResult:(PHFetchResult *)result allowPickingVideo:(BOOL)allowPickingVideo allowPickingImage:(BOOL)allowPickingImage completion:(void (^)(NSArray<LMAssetModel *> *))completion {
    NSMutableArray *photoArr = [NSMutableArray array];
    [result enumerateObjectsUsingBlock:^(PHAsset *asset, NSUInteger idx, BOOL * _Nonnull stop) {
        LMAssetModel *model = [self assetModelWithAsset:asset allowPickingVideo:allowPickingVideo allowPickingImage:allowPickingImage];
        if (model) {
...
- (void)getAssetFromFetchResult:(PHFetchResult *)result atIndex:(NSInteger)index allowPickingVideo:(BOOL)allowPickingVideo allowPickingImage:(BOOL)allowPickingImage completion:(void (^)(LMAssetModel *))completion {
    PHAsset *asset;
    @try {
        asset = result[index];
    }
...
- (LMAssetModel *)assetModelWithAsset:(PHAsset *)asset allowPickingVideo:(BOOL)allowPickingVideo allowPickingImage:(BOOL)allowPickingImage {
    BOOL canSelect = YES;
    ...
    LMAssetModel *model;
    LMAssetModelMediaType type = [self getAssetType:asset];
    if (!allowPickingVideo && type == LMAssetModelMediaTypeVideo) return nil;
    if (!allowPickingImage && type == LMAssetModelMediaTypePhoto) return nil;
    if (!allowPickingImage && type == LMAssetModelMediaTypePhotoGif) return nil;
    
    PHAsset *phAsset = (PHAsset *)asset;
    LMImagePicker *imagePicker = [LMImagePicker sharedImagePicker];
    if (imagePicker.hideWhenCanNotSelect) {
        // 过滤掉尺寸不满足要求的图片
        if (![self isPhotoSelectableWithAsset:phAsset]) {
            return nil;
        }
    }
    NSString *timeLength = type == LMAssetModelMediaTypeVideo ? [NSString stringWithFormat:@"%0.0f",phAsset.duration] : @"";
    timeLength = [self getNewTimeFromDurationSecond:timeLength.integerValue];
    model = [LMAssetModel modelWithAsset:asset type:type timeLength:timeLength];
    return model;
}

- (LMAssetModelMediaType)getAssetType:(PHAsset *)asset {
    LMAssetModelMediaType type = LMAssetModelMediaTypePhoto;
    PHAsset *phAsset = (PHAsset *)asset;
...
- (void)getPhotosBytesWithArray:(NSArray *)photos completion:(void (^)(NSString *totalBytes))completion {
    ...
    for (NSInteger i = 0; i < photos.count; i++) {
        LMAssetModel *model = photos[i];
        PHImageRequestOptions *options = [[PHImageRequestOptions alloc] init];
        options.resizeMode = PHImageRequestOptionsResizeModeFast;
        options.networkAccessAllowed = YES;
        if (model.type == LMAssetModelMediaTypePhotoGif) {
        ...
        [[PHImageManager defaultManager] requestImageDataForAsset:model.asset options:options resultHandler:^(NSData *imageData, NSString *dataUTI, UIImageOrientation orientation, NSDictionary *info) {
            if (model.type != LMAssetModelMediaTypeVideo) dataLength += imageData.length;
            assetCount ++;
...
- (int32_t)getPhotoWithAsset:(PHAsset *)asset completion:(void (^)(UIImage *, NSDictionary *, BOOL isDegraded))completion {
    CGFloat fullScreenWidth = LMScreenWidth;
...
- (int32_t)getPhotoWithAsset:(PHAsset *)asset photoWidth:(CGFloat)photoWidth completion:(void (^)(UIImage *photo,NSDictionary *info,BOOL isDegraded))completion progressHandler:(void (^)(double progress, NSError *error, BOOL *stop, NSDictionary *info))progressHandler networkAccessAllowed:(BOOL)networkAccessAllowed {
    CGSize imageSize;
    if (photoWidth < LMScreenWidth && photoWidth < _photoPreviewMaxWidth) {
        imageSize = AssetGridThumbnailSize;
    } else {
        PHAsset *phAsset = (PHAsset *)asset;
        CGFloat aspectRatio = phAsset.pixelWidth / (CGFloat)phAsset.pixelHeight;
        CGFloat pixelWidth = photoWidth * LMScreenScale * 1.5;
        // 超宽图片
        if (aspectRatio > 1.8) {
    ...
    int32_t imageRequestID = [[PHImageManager defaultManager] requestImageForAsset:asset targetSize:imageSize contentMode:PHImageContentModeAspectFill options:option resultHandler:^(UIImage *result, NSDictionary *info) {
        ...
        if ([info objectForKey:PHImageResultIsInCloudKey] && !result && networkAccessAllowed) {
            ...
            [[PHImageManager defaultManager] requestImageDataForAsset:asset options:options resultHandler:^(NSData *imageData, NSString *dataUTI, UIImageOrientation orientation, NSDictionary *info) {
                ...
                if (![LMImagePicker sharedImagePicker].notScaleImage) {
...
/// Get postImage / 获取封面图
- (void)getPostImageWithAlbumModel:(LMAlbumModel *)model completion:(void (^)(UIImage *))completion {
    id asset = [model.result lastObject];
    if (![LMImagePicker sharedImagePicker].sortAscendingByModificationDate) {
        asset = [model.result firstObject];
    }
    [[LMPhotoManager manager] getPhotoWithAsset:asset photoWidth:80 completion:^(UIImage *photo, NSDictionary *info, BOOL isDegraded) {
...
- (BOOL)isPhotoSelectableWithAsset:(PHAsset *)asset {
    ...
    if ([LMImagePicker sharedImagePicker].minPhotoWidthSelectable > photoSize.width || [LMImagePicker sharedImagePicker].minPhotoHeightSelectable > photoSize.height) {
        return NO;
    }
    return YES;
}
...
- (LMAlbumModel *)modelWithResult:(PHFetchResult *)result name:(NSString *)name isCameraRoll:(BOOL)isCameraRoll needFetchAssets:(BOOL)needFetchAssets {
    LMAlbumModel *model = [[LMAlbumModel alloc] init];
...
- (LMAssetModel *)createModelWithAsset:(PHAsset *)asset {
    LMAssetModelMediaType type = [[LMPhotoManager manager] getAssetType:asset];
    NSString *timeLength = type == LMAssetModelMediaTypeVideo ? [NSString stringWithFormat:@"%0.0f",asset.duration] : @"";
    timeLength = [[LMPhotoManager manager] getNewTimeFromDurationSecond:timeLength.integerValue];
    LMAssetModel *model = [LMAssetModel modelWithAsset:asset type:type timeLength:timeLength];
    return model;
}
